...
cvs.width = Math.min(700, window.innerWidth)
cvs.height = 400
...
class Point {
  ...
  update (delta) {
    ...
    if (this.x >= cvs.width) {
      this.px = cvs.width + (cvs.width - this.px) * bounce
      this.x = cvs.width
    } else if (this.x <= 0) {
      this.px *= -1 * bounce
      this.x = 0
    ...
    if (this.y >= cvs.height) {
      this.py = cvs.height + (cvs.height - this.py) * bounce
      this.y = cvs.height
    } else if (this.y <= 0) {
      this.py *= -1 * bounce
      this.y = 0
...
class Cloth {
  constructor (free) {
    ...
    let startX = cvs.width / 2 - clothX * spacing / 2
...
cvs.onmousemove = setMouse

cvs.onmouseup = () => (mouse.down = false)
